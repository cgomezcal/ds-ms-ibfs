...
func main() {
	...
	execHandler := func(ctx context.Context, msg kafka.Message) error {
		...
		logger.Info("execute request forwarded", "topic", msg.Topic, "offset", msg.Offset, "status", resp.StatusCode, "key", payload.Key, "type", payload.Type)
		return nil
	}
	...
	respHandler := func(ctx context.Context, msg kafka.Message) error {
		...
		if payload.Status == "success" {
			fmt.Printf("[execute_transaction_response] status=success node=%s tx=%s type=%s data=%s key=%s steps=%d at=%s\n", payload.NodeID, payload.TxHash, payload.Type, payload.Data, payload.Key, stepCount, payload.Timestamp)
		} else {
			fmt.Printf("[execute_transaction_response] status=%s node=%s error=%s type=%s data=%s key=%s steps=%d at=%s\n", payload.Status, payload.NodeID, payload.Error, payload.Type, payload.Data, payload.Key, stepCount, payload.Timestamp)
		}
		return nil
	}
